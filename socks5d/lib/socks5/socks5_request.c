...
unsigned RequestReadRun(void *data) {
   ...
   if (null == remoteConnection){
    ...
    return SELECTOR_STATUS_SUCCESS == SelectorSetInterestKey(data,SELECTOR_OP_NOOP) ? CS_ESTABLISH_CONNECTION : CS_ERROR;
}
...
unsigned RequestWriteRun(void *data) {
    ...
    if (!BufferCanRead(d->WriteBuffer))
    {
        // TODO ver esto
        if (
                SELECTOR_STATUS_SUCCESS == SelectorSetInterest(selector, connection->ClientTcpConnection->FileDescriptor,SELECTOR_OP_READ) &&
                SELECTOR_STATUS_SUCCESS == SelectorSetInterest(selector, connection->RemoteTcpConnection->FileDescriptor,SELECTOR_OP_READ)
        ) {
            return CS_CONNECTED;
        }
        return CS_ERROR;

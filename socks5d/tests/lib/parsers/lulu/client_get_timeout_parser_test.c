...
START_TEST(Reset_Succeeds)
{
        ...
        ck_assert_int_eq(parser.State, GetTimeoutGet);
...
START_TEST(Feed_NullParser_Fails)
{
    ...
    // Assert
    ck_assert_int_eq(state, GetTimeoutInvalidState);
}
END_TEST
...
START_TEST(Feed_Complete_Succeeds)
{
    ...
    // Act
    for (int i=0; i < (int)sizeof(message)/(int) sizeof(message[0]) && state != GetTimeoutInvalidState; i++ ){
        state = ClientGetTimeoutParserFeed(&parser, message[i]);
    }

    // Assert
    ck_assert_int_eq(state, GetTimeoutFinished);
}
END_TEST
...
START_TEST(Feed_Get_Succeeds)
{
    ...
    // Assert
    ck_assert_int_eq(state, GetTimeout);
}
END_TEST
...
START_TEST(Feed_Timeout_Succeeds)
{
    ...
    // Assert
    ck_assert_int_eq(state, GetTimeoutCRLF);
}
END_TEST
